...
In this repository you will find a basic analysis pipeline for the Spanish-Rioplatense SWOW project which allows you to import an preprocessing the data as well as compute some basic statistics.
...
Currently word association and participant data is available for 1915 cues. The data consists of over +300k  reponses collected between 2014 and 2018. They are currently submitted for publication. Note that the final version is subject to change.
...
### Preprocessed data
The preprocessed data consist of normalizations of cues and responses by spell-checking them, correcting capitalization and Americanizing. This file is generated by the [preprocessData.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/preprocessData.R) script.
...
In many cases, this preprocessed data is used to derive the associative strengths (i.e. the conditional probability of a response given a cue). These data can be derived using the [createAssoStrengthTable.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/createAssoStrengthTable.R) script. 


## Graphs
Use [createSWOWENGraph.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/createSWOWENGraph.R) to extract the largest strongly connected component  for graphs based on the first response (R1) or all responses (R123). The results are written to [output/adjacencyMatrices](https://github.com/almadana/SWOWES-UY-2018/blob/master/output/adjacencyMatrices) and consist of a file with labels and a sparse file consisting of three values corresponding to row- and column-indices followed by the association frequencies.
...
## Derived statistics
### Response statistics
Use [createResponseStats.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/createResponseStats.R) to calculate a number of response statistics. Currently the script calculates the number of types, tokens and hapax legomena responses (responses that only occur once). The results can be found in the `output` directory.
### Cue statistics
Use [createCueStats.R](https://github.com/almadana/SWOWES_UY-2018/blob/master/R/createCueStats.R) to calculate cue statistics. Only words that are part of the strongly connected component are considered. Results are provided for the R1 graph and the graph with all responses (R123). The file includes the following:
...
A histogram of the response coverage for R1 and R123 graphs can be obtained from the script [plotCoverage.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/plotCoverage.R). Vocabulary growth curves can be obtained with [plotVocabularyGrowth.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/plotVocabularyGrowth.R).
...
An example of this analysis is available in [calculateR12ResponseChaining.R](https://github.com/almadana/SWOWES-UY-2018/blob/master/R/calculateR12ResponseChaining.R).
...
* [EnglishProperNames.txt](https://github.com/almadana/SWOWES-UY-2018/blob/master/data/dictionaries/SpanishProperNames.txt): List of proper names that should not be corrected when found as response
* [EnglishCustomDict.txt](https://github.com/almadana/SWOWES-UY-2018/blob/master/data/dictionaries/SpanishCustomDict.txt) responses that are manually checked. The data in these files take priority over automated (and sometimes faulty) spell-checking. As such, exceptions that should not be touched can be easily included by given including the original response and a correction that is identical.
